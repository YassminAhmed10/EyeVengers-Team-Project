...
function fmtGender(g) {
  if (g === 0 || g === "0" || /^m/i.test(g)) return "Male";
  if (g === 1 || g === "1" || /^f/i.test(g)) return "Female";
  return g || null;
}
...
export default function PatientMedicalRecord() {
  ...
  const load = async () => {
    try {
      ...
      try {
        ...
        checkData = chk;
      } catch { setData(localFallback()); setError("Could not reach the server."); return; }
      ...
      if (checkData.exists) {
        ...
        try {
          ...
          if (checkData.patientIdentifier) localStorage.setItem("patientIdentifier", checkData.patientIdentifier);
        } catch { setError("Could not load full record."); setData(localFallback()); }
      } else {
        try {
          ...
          console.log("[PatientEMRPage] API response from appointment-info:", info);
          ...
          setData({ ...info, patientIdentifier: idToUse, complaints:[], histories:[], investigations:[], eyeExaminations:[], operations:[], prescriptions:[], diagnoses:[], medicalTestFiles:[] });
        } catch (err) { 
          console.error("[PatientEMRPage] Error fetching appointment-info:", err);
          setData(localFallback()); 
        }
      }
    } catch { setData(localFallback()); setError("Could not load full record."); }
  ...
  const localFallback = () => ({
    ...
    contactNumber: localStorage.getItem("patientPhone") || "",
  ...
  // counts for tabs
  const counts = { complaint: complaints.length, history: histories.length, eye: eyeExams.length, inv: investigations.length, rx: prescriptions.length, ops: operations.length, dx: diagnoses.length, images: images.length };
        ...
        {tab === "info" && (
          <div className="pmr-panel">
            ...
            <div className="pmr-panel-body">
              ...
              {(insurance || insId || policy || coverage || emgName || emgPhone) && <>
                <div className="pmr-hr" />
                <div className="pmr-sub">Insurance & Emergency</div>
                <div className="pmr-info-grid">
