...
function EMRPage() {
    ...
    const currentPatientId = localStorage.getItem("patientId"); // للمريض

    // السماح للطبيب والمريض برؤية السجل الطبي
    // المريض يرى سجله الخاص فقط
    if (userRole !== "Doctor" && userRole !== "Patient") {
        navigate("/login");
        return null;
    }

    // إذا كان المريض، تأكد أنه يرى سجله فقط
    ...
    useEffect(() => {
        const fetchPatientData = async () => {
            try {
                ...
                // استخدم patientId من الـ params أو من localStorage للمريض
                const idToFetch = patientId || (userRole === "Patient" ? currentPatientId : null);
                
                if (!idToFetch) {
                    throw new Error("No patient ID found");
                }

                // تحقق من وجود السجل الطبي
                const checkResult = await emrService.checkMedicalRecordExists(idToFetch);
                setMedicalRecordExists(checkResult.exists);

                if (checkResult.exists) {
                    // إذا وجد سجل طبي موجود، جلب البيانات من قاعدة البيانات
                    const medicalRecord = await emrService.getPatientMedicalRecord(idToFetch);
                    const formattedData = formatMedicalRecordData(medicalRecord);
                    setPatientData(formattedData);
                } else {
                    // إذا لم يكن هناك سجل طبي، حاول جلب البيانات من المواعيد
                    try {
                        const appointments = await appointmentsAPI.getAll();
                        const patientAppointments = appointments.filter(a => 
                            a.patientId.toString() === idToFetch || 
                            a.patientName.toLowerCase() === idToFetch.toLowerCase()
                        );
                        ...
                        console.warn("Could not fetch appointment data");
                        setPatientData({
                            name: "Patient",
                            patientID: idToFetch || "Unknown",
                            age: "Not specified",
                ...
                const idToFetch = patientId || currentPatientId;
        ...
        fetchPatientData();
    }, [patientId, location, currentPatientId, userRole]);
    ...
    const formatAppointmentData = (appointmentData) => {
        ...
        return {
            ...
            histories: [{
                PastMedicalHistory: appointmentData.eyeSurgeries || appointmentData.otherEyeSurgeries || "",
                FamilyHistory: appointmentData.familyEyeDiseases || appointmentData.otherFamilyDiseases || "",
                Allergies: appointmentData.eyeAllergies || appointmentData.otherAllergies || "",
                chronicDiseases: appointmentData.chronicDiseases || "",
                currentMedications: appointmentData.currentMedications || "",
                eyeSurgeries: appointmentData.eyeSurgeries || appointmentData.otherEyeSurgeries || "",
                familyEyeDiseases: appointmentData.familyEyeDiseases || appointmentData.otherFamilyDiseases || "",
                visionSymptoms: appointmentData.visionSymptoms || "",
                CreatedAt: appointmentData.appointmentDate
            }],
            investigations: [],
            eyeExaminations: [],
