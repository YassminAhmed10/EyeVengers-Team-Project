...
import { FaCalendarCheck, FaExclamationTriangle, FaInfoCircle, FaUserPlus, FaBan } from 'react-icons/fa';
...
const ReceptionistHeader = ({ toggleDarkMode, darkMode, toggleSidebar }) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const storedNotificationIds = JSON.parse(localStorage.getItem('receptionistNotificationIds') || '[]');
      const newNotificationIds = [];

      // Check for online appointment requests (status 0 and appointmentType 'online')
      const onlineRequests = data.filter(apt => 
        apt.appointmentType === 'online' && apt.status === 0
      );

      onlineRequests.forEach(request => {
        const notifId = `online-${request.appointmentId}`;
        if (!storedNotificationIds.includes(notifId)) {
          newNotifications.push({
            id: notifId,
            type: 'appointment',
            title: 'New Online Appointment Request',
            message: `${request.patientName} requested an appointment for ${new Date(request.appointmentDate).toLocaleDateString()}`,
            time: 'Just now',
            unread: true,
      ...
      // Check for cancelled appointments (status 2)
      const cancelledAppointments = data.filter(apt => apt.status === 2);
      
      cancelledAppointments.forEach(apt => {
        const notifId = `cancelled-${apt.appointmentId}`;
        if (!storedNotificationIds.includes(notifId)) {
          newNotifications.push({
            id: notifId,
            type: 'warning',
            title: 'Appointment Cancelled',
            message: `${apt.patientName} cancelled their appointment scheduled for ${new Date(apt.appointmentDate).toLocaleDateString()}`,
            time: 'Recently',
            unread: true,
      ...
      if (newNotifications.length > 0) {
        ...
        localStorage.setItem('receptionistNotificationIds', 
          JSON.stringify([...storedNotificationIds, ...newNotificationIds]));
      }
    } catch (error) {
  ...
  const handleNotificationClick = (notification) => {
    // Navigate to appointments page if it's an online request or cancellation
    if (notification.type === 'appointment' || notification.type === 'warning') {
  ...
  const clearAllNotifications = (e) => {
    ...
    localStorage.removeItem('receptionistNotificationIds');
  };
