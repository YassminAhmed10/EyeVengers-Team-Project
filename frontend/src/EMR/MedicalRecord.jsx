...
const MedicalRecord = ({ patientName, patientId, initialPatientData, fromAppointment }) => {
    ...
    useEffect(() => {
        if (initialPatientData) {
            // Check if data comes from histories array (database format)
            if (initialPatientData.histories && initialPatientData.histories.length > 0) {
            ...
            // Check if data comes directly from appointment (appointment format)
            else if (initialPatientData.eyeAllergies || initialPatientData.chronicDiseases || 
                     initialPatientData.currentMedications || initialPatientData.eyeSurgeries ||
                     initialPatientData.familyEyeDiseases || initialPatientData.visionSymptoms) {
                setHistoryData({
